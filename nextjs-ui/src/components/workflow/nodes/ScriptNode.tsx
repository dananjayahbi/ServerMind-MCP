...
export function ScriptNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`min-w-[240px] max-w-[320px] rounded-xl border-2 transition-all ${selected ? "border-[#A78BFA] shadow-[0_0_10px_#A78BFA]" : "border-[#A78BFA]/30"} bg-[#130D1A] p-3`}>
      <Handle type="target" position={Position.Top} className="!bg-[#A78BFA] !border-[#0D0D0D] !w-3 !h-3" />
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-[#A78BFA] !border-[#0D0D0D] !w-3 !h-3" />
    </div>
  );
}
